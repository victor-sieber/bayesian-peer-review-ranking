...
Grant peer review often requires funding decisions based on only a small number
of evaluations per proposal, making the uncertainty of resulting rankings
difficult to assess. This report studies the 2026 CRS Seed Grant evaluation,
where 42 proposals were each reviewed twice by a panel of nine reviewers. A
Bayesian hierarchical ranking model separates proposal effects,
reviewer-associated scoring tendencies and residual review-level variation,
while representing ranking uncertainty through posterior rank distributions.

A CRS-specific cross-classified model reproduced the four proposals selected
under the final CRS decision rule, but substantial uncertainty remained in the
ordering of individual proposals. The model was computationally stable and
insensitive to the tested prior specifications. Posterior predictive checks
showed good agreement with observed review patterns, including average
reviewer disagreement, while the unbounded Gaussian representation of the
five-point scores remained an explicit modelling caveat.

Changing the evaluation outcome from the holistic overall grade to an
equal-weight average of four supporting criteria produced materially different
proposal rankings, indicating that the definition of the evaluation outcome can
have a stronger influence than the choice between the tested Bayesian model
specifications. Finally, the reviewer--proposal allocation was represented as
a bipartite graph to evaluate hypothetical additional reviews using Bayesian
A-, D- and E-optimality criteria. The analysis identifies assignments that
would provide the largest expected reduction in ranking uncertainty under the
fitted model, while requiring operational constraints such as reviewer
expertise and availability before practical use.

Overall, the results demonstrate how Bayesian ranking can complement
transparent evaluation rules by quantifying uncertainty and identifying where
additional review effort could provide the greatest statistical benefit.
...
The holistic overall grade is the response for the main Bayesian analysis. The
...
preferred scoring rule. V0 reproduces the continuous Heyard reference
specification, while the simpler cross-classified V1 model is used as the main
CRS model. After comparing the resulting rankings, Chapter 5 evaluates the
computational stability, posterior predictive behaviour and prior sensitivity
of V1.

Chapter 6 then treats the reviewer--proposal assignment structure as a
statistical-design problem. It asks which one currently unobserved assignment
would most reduce posterior uncertainty under V1 and compares A-, D- and
E-optimality criteria. Because reviewer expertise, conflicts and eligibility
are not available, these candidate assignments are interpreted only as
statistical information rankings. The final discussion brings together the
ranking, validation, criterion and review-design results and derives practical
...
The analysis then considers a deliberately restricted prospective question:
...
reduction in uncertainty under V1? The analysis uses established
optimal-design criteria rather than introducing a new measure of uncertainty,
and compares several such criteria because different notions of statistical
precision need not prefer the same additional review.
...
To make this connection explicit, define
...
that is, the first 42 diagonal entries are $\tau_\theta^{-2}$ and the
remaining nine are $\tau_b^{-2}$.
...
denote the sampled proposal, reviewer and residual scale parameters. The
corresponding prior precision matrix is
...
Targeting proposals with large observed reviewer disagreement would answer a
different question from the one studied here. Such a rule would prioritize
where an additional opinion may be substantively useful, whereas the
A-, D- and E-optimality criteria quantify reduction in posterior contrast
uncertainty under V1. Reviewer disagreement is therefore not used as an
alternative design criterion in this chapter.
...
ranking will, however, depend on the fitted variance structure and on the
uncertainty criterion chosen.

The present analysis considers only one additional review. With a budget of
several reviews, the assignments could be selected sequentially, updating the
information structure after each added edge, or optimized jointly as a set.
The single-edge rankings should therefore not be interpreted as a complete
multi-review allocation.

A prospective implementation would also restrict the candidate set before
optimization. Reviewer expertise, conflicts of interest, workload limits and
availability could be encoded as eligibility constraints, after which the
statistical criterion would rank only feasible assignments. Finally, the
results are conditional on V1 and on the holistic overall grade as the main
response. Alternative outcome models, including ordinal models, or
decision-specific objectives could produce different preferred assignments.
...
The review-design analysis connects this uncertainty to the structure of data
collection. Under V1, an additional assignment changes the graph Laplacian and
therefore the conditional posterior covariance. This makes it possible to rank
potential additional reviews by their statistical information value. The
preferred edge is nevertheless objective-dependent, particularly under
E-optimality, and a practical allocation would first need to incorporate
reviewer eligibility and expertise.
...
If additional review capacity is available, it can be allocated deliberately
rather than uniformly. A practical version of the Chapter 6 calculation would
first remove ineligible reviewer--proposal pairs based on expertise, conflicts,
availability and workload, and would then optimize a pre-specified criterion
such as A-optimality over the remaining candidates. The specific reviewer IDs
identified in the retrospective calculation should not be used as direct
assignment recommendations because those operational constraints were not part
of the analysis.
...
expertise, conflicts of interest and other operational constraints are absent
from the candidate set.

Third, V1 treats the five-point holistic grade as Gaussian with homogeneous
...
Fourth, the criterion-average analysis is an equal-weight counterfactual. It
...
decision rule. Similarly, the single-review design calculation conditions on
the current V1 posterior and evaluates a covariance update with fixed scale
parameters within each draw; it does not model how an observed future grade
would subsequently update the full hierarchical model.
...
For review design, the next step is to incorporate eligibility constraints and
extend the one-edge calculation to several additional reviews. Sequential
selection would recompute the information structure after each added review,
while joint optimization could target the best set under a fixed review
budget. Decision-focused utilities could also be considered when the primary
goal is resolving uncertainty near a qualification or funding boundary rather
than reducing global ranking uncertainty.
...
E-optimality can therefore be used to evaluate the statistical value of one
additional review, although the preferred assignment depends on the objective
and must be restricted by operational eligibility in practice. The main
practical takeaway is to combine transparent decision rules with explicit
uncertainty summaries and, where additional review effort is available, to
allocate that effort according to a pre-specified statistical objective among
feasible reviewers.
