...
Grant peer review often requires decisions from only a small number of evaluations per proposal. This report studies the 2026 CRS Seed Grant evaluation, in which 42 proposals were each reviewed twice by a panel of nine reviewers. A Bayesian hierarchical ranking model separates proposal effects, reviewer-associated scoring tendencies and residual review-level variation, and represents ranking uncertainty through posterior rank distributions.

A CRS-specific cross-classified model produced the same four highest posterior-mean proposals as the four proposals that qualified under the final CRS rule, but exact latent ranks remained highly uncertain. The model was computationally stable and insensitive to the tested scale priors. Its posterior median variance shares were 0.050 for proposals, 0.026 for reviewers and 0.896 for residual variation. Posterior predictive checks reproduced the observed mean, dispersion and average reviewer disagreement reasonably well, while the unbounded Gaussian treatment of the five-point score remained an explicit modelling caveat.

An equal-weight average of the four supporting criteria produced materially different proposal ordering, showing that the definition of the evaluation outcome matters more than the distinction between the two tested Bayesian model specifications. Finally, the reviewer--proposal assignment structure was represented as a bipartite graph and used to evaluate one hypothetical additional review. A- and D-optimality gave similar candidate rankings, whereas E-optimality emphasized a different uncertainty direction. The results support Bayesian ranking as an uncertainty-aware supplement to transparent evaluation rules and show how additional review effort can be targeted statistically once operational eligibility constraints are specified.
