...
<!-- TODO:
Compare the ranking of candidate single reviews under A-, D- and E-optimality.

Assess whether the same reviewer--proposal assignments remain highly informative
across the three criteria or whether the preferred edge depends materially on
the definition of uncertainty.

Summarize agreement between the candidate rankings, for example using rank
correlations and the overlap among the highest-ranked candidate edges.

Visualize the candidate-edge utilities, preferably using a reviewer--proposal
heatmap for the primary A-optimality criterion and a compact comparison of the
three criteria.

Compare the model-based recommendation with a simple intuitive benchmark, such
as targeting a proposal with high observed reviewer disagreement, while making
clear that disagreement and expected information gain are conceptually
...
<!-- TODO:
Discuss which parts of the one-step framework depend on the specific CRS design
and which extend naturally to other sparse peer-review settings.

Explain that the present analysis considers only one additional review. Extending
the approach to a budget of multiple reviews would require sequential or joint
optimization because each added assignment changes the information structure
for subsequent assignments.

Discuss how reviewer eligibility, expertise, workload limits and
conflict-of-interest constraints could be incorporated into the candidate set
in a prospective application.

Clarify that the present result is conditional on the fitted V1 model and the
observed reviewer--proposal design, and that other outcome models or ranking
objectives could lead to different preferred assignments.
-->
